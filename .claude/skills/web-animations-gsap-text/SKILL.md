...
Use `yPercent` (a transform) rather than `y` in pixels so the offset scales with the line height and stays on the compositor. See [performance](../web-animations-gsap-performance/SKILL.md).
...
See [react](../web-animations-gsap-react/SKILL.md) for the `useGSAP` scope model and [scroll-trigger](../web-animations-gsap-scroll-trigger/SKILL.md) for trigger boundaries.
