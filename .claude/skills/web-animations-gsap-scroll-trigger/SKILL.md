...
For SplitText and font-dependent layouts, refresh after `document.fonts.ready` (see [text](../web-animations-gsap-text/SKILL.md)).
...
See [react](../web-animations-gsap-react/SKILL.md) for the full `useGSAP` cleanup model and [timelines](../web-animations-gsap-timelines/SKILL.md) for driving a timeline from a single trigger.
