...
This pairs with [functions](../shared-clean-code-functions/SKILL.md) (an error handler is a function that does one thing — handling errors) and [objects-and-data](../shared-clean-code-objects-and-data/SKILL.md) (the wrapper below is an object that hides a library behind a contract). Error and result types follow [type-safety](../shared-type-safety/SKILL.md).
...
The wrapper is also the natural seam for testing (mock the gateway, not the network) and the only file you touch when you replace Gringotts with a rival goblin bank. This is the same principle as *program to a contract* in [objects-and-data](../shared-clean-code-objects-and-data/SKILL.md): the gateway is your contract, the SDK is a hidden implementation detail.
...
- **Catch narrowly, act on type.** In TS every `catch` binding is `unknown`. Narrow with `instanceof` before you touch it; never assume it's an `Error`, and never `as`-cast it (see [type-safety](../shared-type-safety/SKILL.md)). Re-throw what you didn't mean to handle.
...
For a single value that legitimately may be absent, don't reach for `null` either — return `T | undefined` and let the caller narrow it (see [type-safety](../shared-type-safety/SKILL.md)). Where "not found" is genuinely exceptional for the caller, pair the lookup with a `requireX` variant that throws (like `requireVault` above), so the happy path stays flat.
