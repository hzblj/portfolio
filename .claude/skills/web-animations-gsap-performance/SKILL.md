...
- **Prefer numeric `scrub` smoothing.** `scrub: 1` eases progress over ~1s, which both feels premium and decouples animation from raw scroll frequency. See [scroll-trigger](../web-animations-gsap-scroll-trigger/SKILL.md).
...
See [scroll-trigger](../web-animations-gsap-scroll-trigger/SKILL.md) for scrub/pin tuning, [text](../web-animations-gsap-text/SKILL.md) for the reduced-motion text fallback, and [react](../web-animations-gsap-react/SKILL.md) for where to place `matchMedia` inside `useGSAP`.
