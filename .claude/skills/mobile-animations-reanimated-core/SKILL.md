...
Animate **props** (not styles) on the UI thread — SVG attributes, `TextInput.text`, scroll offsets, etc. Requires an animated component created with `Animated.createAnimatedComponent`. See [text-and-numbers](../mobile-animations-reanimated-text-and-numbers/SKILL.md) for the text pattern.
...
- Wrap the app root in `GestureHandlerRootView` (from `react-native-gesture-handler`) — see [gestures](../mobile-animations-reanimated-gestures/SKILL.md).
