...
A CSS **transition** only animates when a value *changes* — it never fires on first paint. So a transition-based swap is skip-on-load for free: keep both states mounted and cross-fade on change (this is the icon cross-fade in [motion](../shared-ui-motion/SKILL.md)).
