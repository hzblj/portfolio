...
For per-frame side effects at a threshold (e.g. haptics as you cross a snap point), pair the gesture with a [`useAnimatedReaction`](../mobile-animations-reanimated-core/SKILL.md) instead of calling `runOnJS` inside `onUpdate`.
