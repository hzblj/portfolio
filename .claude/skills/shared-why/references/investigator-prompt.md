...
Fill in the placeholders and send this as the `Agent` prompt. Append the matching row from the roster table in [`../../shared-why/SKILL.md`](../../shared-why/SKILL.md) as the assigned-source section. If the target code looks defensive (null guards, retries, timeouts, rate limits, feature flags, OOM handlers), add the incident instruction at the end.
