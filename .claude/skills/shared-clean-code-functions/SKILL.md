...
Good functions lean hard on good names — see [meaningful-names](../shared-clean-code-meaningful-names/SKILL.md) first. For how functions should signal failure, see [error-handling](../shared-clean-code-error-handling/SKILL.md).
...
Objects also make the call site self-documenting: `awardPoints({ wizard: harry, house: gryffindor, points: 10 })` reads far better than three bare positional values. See [meaningful-names](../shared-clean-code-meaningful-names/SKILL.md) for naming the wrapper type well.
