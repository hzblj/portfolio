...
For splitting the logic itself into small honest functions, see [functions](../shared-clean-code-functions/SKILL.md); for what to call any of these types, see [meaningful-names](../shared-clean-code-meaningful-names/SKILL.md). Types in every example follow the strict typing rules in [type-safety](../shared-type-safety/SKILL.md) — `type` over `interface`, no `any`, narrow unions.
...
| Using `interface` for the contract | Use `type` per [type-safety](../shared-type-safety/SKILL.md); classes still `implements` it |
